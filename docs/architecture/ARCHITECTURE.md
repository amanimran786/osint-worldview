...
| **Type-safe data flow**             | Discriminated union markers (`_kind` field), proto-generated typed clients/servers, and exhaustive `switch` matching provide compile-time checks across marker types, service domains, and map layers. Adding a new data type produces compiler errors at unhandled typed call sites.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
**Negative sentinel handling** - when Redis contains a `__WM_NEG__` sentinel, the bootstrap endpoint treats that key as unavailable and lets the panel use its normal fallback path.

**Per-tier CDN caching** - the fast tier uses a 10-minute shared-cache lifetime and the slow tier uses a one-hour shared-cache lifetime. CDN-specific headers permit longer slow-tier caching. Failed or completely unseeded bootstrap responses use `no-store` and HTTP 503.
...
This reduces the number of independent startup requests when the production cache is populated. Panels still retain their individual fallback paths.
...
Production is designed to run the seed scripts as independently scheduled Railway jobs. The table below is the intended schedule, not evidence that a hosted job is currently active. Deployment approval requires live seed-health and bootstrap probes.
...
WorldView uses focused Vercel Edge Functions as its API layer. Legacy endpoints in `api/*.js` each handle a single data-source concern, while proto-first endpoints use per-domain entry points that import their own handlers. A shared `server/gateway.ts` provides common routing logic. Both generations currently coexist:

- **RSS Proxy** - domain-allowlisted proxy for configured feeds. Relay fallback is available only when the Railway relay is configured.
