...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_metadata = AboutMetadata {
        ...
        copyright: Some("\u{00a9} 2026 Aman Imran".into()),
        website: Some("https://osint-worldview-cyan.vercel.app".into()),
        website_label: Some("WorldView on the web".into()),
        ..Default::default()
    };
    let about_item =
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/amanimran786/osint-worldview");
        }
        #[cfg(feature = "devtools")]
        MENU_HELP_DEVTOOLS_ID => {
